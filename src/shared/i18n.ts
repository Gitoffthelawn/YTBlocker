...
const messages: Record<string, Dict> = {
  ...
  'sync.disable.label': {
    ja: 'FIREFOXアカウントにNGルールを含むオプションデータを保存しない（ローカル保存）',
    en: 'Do not save option data, including block rules, to Firefox Account (local storage only)',
    de: 'Optionsdaten inkl. Sperrregeln nicht im Firefox-Konto speichern (nur lokal)',
    it: 'Non salvare i dati delle opzioni, incluse le regole di blocco, nell’account Firefox (solo locale)',
    fr: 'Ne pas enregistrer les données d’options, y compris les règles de blocage, dans le compte Firefox (stockage local uniquement)',
    ko: '차단 규칙을 포함한 옵션 데이터를 FIREFOX 계정에 저장하지 않음(로컬 저장)',
    'zh-CN': '不将包含屏蔽规则的选项数据保存到 FIREFOX 账户（仅本地保存）',
    'zh-TW': '不將包含封鎖規則的選項資料保存到 FIREFOX 帳戶（僅本機保存）',
  },
  'log.disable.label': {
    ja: 'リアルタイムブロックログを表示しない（パフォーマンス優先）',
    en: 'Do not show the real-time block log (prioritize performance)',
    de: 'Echtzeit-Sperrprotokoll nicht anzeigen (Leistung priorisieren)',
    it: 'Non mostrare il registro dei blocchi in tempo reale (priorità alle prestazioni)',
    fr: 'Ne pas afficher le journal des blocages en temps réel (priorité aux performances)',
    ko: '실시간 차단 로그를 표시하지 않음(성능 우선)',
    'zh-CN': '不显示实时屏蔽日志（优先性能）',
    'zh-TW': '不顯示即時封鎖紀錄（優先效能）',
  },
  'shorts.label': {
  ...
  'form.sampleLabel': {
    ...
    en: 'Test sample text (not saved; video title or channel name)  e.g. test ch',
    de: 'Test-Beispieltext (wird nicht gespeichert; Videotitel oder Kanalname)  z. B. test ch',
    it: 'Testo di esempio per il test (non salvato; titolo del video o nome del canale)  es. test ch',
    fr: 'Texte d’exemple de test (non enregistré ; titre de la vidéo ou nom de la chaîne)  ex. test ch',
    ko: '테스트용 샘플 문자열(저장되지 않음・동영상 제목 또는 채널명)  예: test ch',
    'zh-CN': '测试用示例文本（不会保存・视频标题或频道名称） 例：test ch',
    'zh-TW': '測試用範例文字（不會保存・影片標題或頻道名稱） 例：test ch',
  },
  'form.regexLabel': {
    ja: '登録する正規表現パターン（これが保存されます）　例 aaaa|bbbb',
    en: 'Regex pattern to register (this is what gets saved)  e.g. aaaa|bbbb',
    de: 'Zu registrierendes Regex-Muster (dies wird gespeichert)  z. B. aaaa|bbbb',
    it: 'Pattern regex da registrare (questo verrà salvato)  es. aaaa|bbbb',
    fr: 'Motif regex à enregistrer (c’est ce qui sera sauvegardé)  ex. aaaa|bbbb',
    ko: '등록할 정규식 패턴(이것이 저장됩니다)  예: aaaa|bbbb',
    'zh-CN': '要注册的正则表达式（这将被保存） 例：aaaa|bbbb',
    'zh-TW': '要註冊的正規表示式（這將被保存） 例：aaaa|bbbb',
  },
  'form.matchNote': {
  ...
  'log.disabledMsg':  { ja: 'ブロックログの表示は無効になっています', en: 'The block log display is disabled', de: 'Die Anzeige des Sperrprotokolls ist deaktiviert', it: 'La visualizzazione del registro dei blocchi è disattivata', fr: 'L’affichage du journal des blocages est désactivé', ko: '차단 로그 표시가 비활성화되어 있습니다', 'zh-CN': '屏蔽日志显示已停用', 'zh-TW': '封鎖紀錄顯示已停用' },
